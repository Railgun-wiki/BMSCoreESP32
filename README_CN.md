...
- **MQTT 遥测**: SmartConfig (EspTouch) 智能配网 + JSON 遥测上报 (电压/电流/SOC/温度)
...
### SPI — DAC8562（第二个通用 SPI host，独立总线）
| 信号 | GPIO |
|------|------|
| DAC_MOSI | 40 |
| DAC_SCLK | 41 |
...
在 ESP32-S3 的 Arduino 核心里，这条 DAC 总线对应第二个通用 SPI host，也就是 `HSPI`。硬件连线保持不变，逻辑上仍与 LCD 独立。
...
| SmartConfig | 原生 | 芯片自带 EspTouch 配网 |
...
- SmartConfig — ESP 原生 EspTouch 一键配网
- AP 名称: `ESP32S3_BMS`
...
- [x] Phase 4: SmartConfig 配网 + MQTT 遥测
